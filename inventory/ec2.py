...
class Ec2Inventory(object):
    ...
    def __init__(self):
        ...
        if self.args.hosts_list:
            return self.list_hosts(self.args.hosts_list)
    ...
    def parse_cli_args(self):
        ...
        parser.add_argument('--hosts-list', metavar='prod|test', action='store',
                           help='List all hosts from specified env')
    ...
    def list_hosts(self, env):
        if self.inventory == self._empty_inventory():
            data = json.loads(self.get_inventory_from_cache())
        else:
            data = self.inventory
 
        for hostname in data['_meta']['hostvars']:
            host = data['_meta']['hostvars'][hostname]

            name = host.get('ec2_tag_Name')

            if name is None:
                name = host.get('ec2_tag_Role')

            if name is None or not host.get('ec2_private_ip_address'):
                continue

            if re.match('^ctf-' + re.escape(env), name):
                print "%-30s %-18s %-18s %-30s" % (name, host.get('ec2_ip_address'), host.get('ec2_private_ip_address'), host.get('ec2_id'))
